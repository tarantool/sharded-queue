...
function g.test_statistics()
    ...
    for i = 1, task_count do
        table.insert(task_pack, queue_conn:call('tube:put', {
            i, { delay = 3 , ttl = 3, ttr = 1}
        })[helper.index.task_id])
    ...
    for i = 1, taken_task_count do
        table.insert(taken_task_pack, queue_conn:call('tube:take', { 0.001 })[helper.index.task_id])
    end
    ...
    for i = 1, done_task_count do
        queue_conn:call('tube:ask', { taken_task_pack[i] })
    end
